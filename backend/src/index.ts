...
app.post("/app/v1/signin", async (req: Request, res: Response) => {
  ...
  /* ================= LOGOUT ================= */
app.post("/app/v1/logout",middleware ,(req: CustomRequest, res: Response) => {
...
app.get("/app/v1/Brain/shareLink" ,middleware, async(req : CustomRequest , res:Response) =>{
        const id = req.userId;
        if (!id) {
...
app.post("/app/v1/Brain/shareLink", middleware, async (req: CustomRequest, res: Response) => {
...
//////////getmsg////////////////////
app.get("/getmessage", middleware, async (req: CustomRequest, res: Response) => {
  const userId = req.userId;
...
//////////////////getusers//////////////////
app.get("/getusers", middleware, async (req: CustomRequest, res: Response) => { 
  try { const userId = req.userId;
     if (!userId) { return res.status(401).json({ message: "Unauthorized" }); } 
     const users = await UserModel.find({ _id: { $ne: userId } }) .select("username _id"); 
     return res.status(200).json({ users : users }); } catch (error) 
     { return res.status(400).json({ message: "Error fetching users", }); } });

/////////sendmsg///////////////
app.post("/sendmsg", middleware, async (req: CustomRequest, res: Response) => {
